...
snapshot with **no new permissions**. Full-fidelity a11y (`Accessibility.getFullAXTree`), trusted
input, and file upload need CDP via `chrome.debugger` — deferred to an opt-in power mode (out of scope
here). The extension service worker sleeps (~30 s idle), so snapshot/ref state must live on the
long-lived side-panel document, not the service worker.
...
handles**. Blog measurements put a text snapshot roughly an order of magnitude (~10–100×) cheaper than
the alternatives — a vision screenshot commonly runs ~10k–50k tokens and a raw-DOM dump a few thousand,
...
- **Auto-wait consensus** (independent validation for `wait_for`): Playwright auto-waits on
  ...
  every action `[PW-Act]`;
  Cypress re-queries the DOM and re-runs assertions until pass/timeout, with fixed `cy.wait(N)` as the
  named anti-pattern `[Cypress-Retry]`; Selenium leaves waits manual and mixing implicit+explicit is a
  documented footgun `[Selenium-Waits]`. "Auto-wait beats hard-sleep" is an industry-wide consensus —
  direct precedent for folding auto-wait **into** katashiro's actions rather than exposing sleeps to
  the agent.
...
- **The `ref` is the primary key; role+name is the human-readable label on it.** Two "Submit" buttons
  ...
  distinct ref.
- **Ref lifetime must be defined explicitly.** A `data-testid` is page-lifetime because a human wrote it
  into the markup `[Cypress-BP]`; katashiro's ref is minted per-snapshot. Default to **snapshot-scoped**
  (MCP-aligned `[PW-MCP]`, simplest): refs are valid within one snapshot; any navigation/DOM mutation
  yields a fresh snapshot with new refs. Cross-snapshot identity is harder and is the class of bug that
  bit the Falcon "Session-not-found" work — snapshot-scoped is the safer default.
...
Rules: label only **interactive/semantically meaningful** elements with a `ref` (labeling every wrapper
bloats the tree for no benefit); label each by (role, accessible name) following Testing Library's
priority ladder (name → label → text → alt/title) `[TL-Queries]`. Refs are **snapshot-scoped** (§2.5)
and reissued on the next snapshot.
...
re-locate round-trip, no layout-shift brittleness. A stale ref (page changed since the snapshot) must
**fail cleanly and prompt a re-snapshot**, never mis-click.
...
### 3.4 Add `wait_for` and fold actionability into actions

`wait_for {selector? | text? | ref?, state?, timeout}` plus internal auto-wait (Playwright's six
actionability checks: attached/visible/stable/receives-events/enabled/editable `[PW-Act]`) before every
`click`/`type`/`navigate`, so
the agent never spends model turns polling with screenshots to check readiness. `wait_for`
...
- `screenshot`: "EXPENSIVE (large image, slow vision reasoning). ONLY when a text snapshot cannot
  answer — visual layout, images/charts/canvas, or elements with no accessible name. NEVER to confirm
  an action succeeded — action tools already return the updated snapshot."
...
optional `concise|detailed` mode `[Anthropic-Tools]`. Raw-HTML dumps are the ~3–5k-token trap
`[token-cost]` and bloat every subsequent turn's context.

### 3.7 Implementation building blocks — build vs borrow

Borrow the hard ARIA computation; write only katashiro-specific glue.

- **`dom-accessibility-api` `[dom-a11y-api]` — BORROW (primary block).** Spec-faithful accessible
  **name + role** (+disabled/inaccessible); it is the engine Testing Library uses. Zero runtime deps,
  no eval, MIT. Ships CJS/ESM only → vendor via a **one-time offline bundle to IIFE `'self'`** (a
  packaging step, not a runtime build system). Do **not** hand-roll accname — it is a gnarly spec.
- **Playwright `ariaSnapshot.ts` `[PW-ariaSnapshot]` — REFERENCE-PORT, don't import.** It already emits
  the `role "name" [ref=eN]` format (Apache-2.0), but is coupled to Playwright's internal `roleUtils`/
  `domUtils`. Copy its tree-walk structure + output format; swap its role/name computation for
  `dom-accessibility-api`.
- **`chrome.automation` `[chrome-automation]` — DON'T (dead end).** The real AXTree via extension API is
  ChromeOS-only/experimental; `getDesktop()` never shipped for desktop Chrome. Compute from the DOM.
...
- **`aria-query` `[aria-query]` — optional.** Static ARIA role metadata; only if the interactability
  filter needs it beyond `getRole`.
- Adjacent (tracked separately): **`@mozilla/readability` + DOMPurify** for a `get_text` tool
  `[Readability]`; **`@dagrejs/dagre`** for the later whiteboard DAG layout `[dagre]`.

**Glue katashiro must write itself (no lib covers it):** (1) ref registry — `WeakMap<Element, refId>` +
monotonic `eN`, resolvable back to the live element; (2) tree walker + snapshot serialization with
visibility/interactability filtering; (3) snapshot-after-action loop + staleness handling; (4) the MCP
tool surface (`snapshot`/`click(ref)`/`type(ref)`/`get_text`) + content-script ↔ side-panel plumbing.
...
### Positive
- Per-observation cost drops roughly an order of magnitude (~10–100×, blog estimates) — text tree vs
  image `[PW-MCP]` `[token-cost]`; no vision inference on the hot path.
- Round-trips roughly halve: snapshot-after-action removes the confirm turn; ref-targeting removes the
  re-locate turn — the two dominant contributors to the measured 46 s turn.
...
### Negative / tradeoffs
- New work: a content-script a11y walker + a snapshot-scoped ref registry on the side-panel document.
- The content-script a11y tree is an **approximation** of the browser's true a11y tree (the full tree is
  CDP-only `[chrome-automation]`); edge cases (poor ARIA, shadow DOM, iframes) may need the power mode.
- Synthetic content-script events are `isTrusted=false`; some strict sites ignore them — trusted input
  is a later CDP upgrade.
...
- **Full CDP `Accessibility.getFullAXTree` via `chrome.debugger` now.** Rejected for P0: needs the
  `debugger` permission, shows the persistent "started debugging this browser" infobar, conflicts with
  open DevTools. Deferred to opt-in power mode; content-script approximation covers most pages with zero
  new permissions.
...
- **In scope (P0, content-script, no new permissions):** `snapshot` + refs, `wait_for`, snapshot-after-
  action, ref-accepting `click`/`type`, description reframing, `read_dom` reshape.
- **Out of scope (later / opt-in power mode):** CDP full a11y tree, trusted input, file upload, network
  interception, PDF — each requires `chrome.debugger` and is a separate decision.
...
- **[PW-ariaSnapshot]** Playwright — `packages/injected/src/ariaSnapshot.ts` (the `role "name" [ref=eN]` generator, Apache-2.0) — https://github.com/microsoft/playwright/blob/main/packages/injected/src/ariaSnapshot.ts
...
- **[PW-Act]** Playwright — Auto-waiting / actionability — https://playwright.dev/docs/actionability
...
- **[dom-a11y-api]** `dom-accessibility-api` (accessible name+role; Testing Library's engine; MIT) — https://github.com/eps1lon/dom-accessibility-api
...
- **[chrome-automation]** `chrome.automation` (ChromeOS-only/experimental — rejected) — https://developer.chrome.com/docs/extensions/reference/api
